...
// Zero String Property
static const aafUID_t					TEST_ZERO_PROP_ID =
{0x7887c134, 0xf265, 0x451e, { 0xaa, 0x91, 0xa8, 0xa6, 0xcd, 0xd7, 0xdc, 0x4f}};
...
static HRESULT addZeroStringPropertyToComponent(IAAFDictionary * const pDict)
{
	//Get the  property def for Component::STR
	IAAFClassDefSP spCD_comp;
	checkResult(pDict->LookupClassDef(AUID_AAFComponent, &spCD_comp));
	...
	IAAFTypeDefSP spTD;
	checkResult(pDict->LookupTypeDef(kAAFTypeID_String, &spTD));

	//	Register the Property
	IAAFPropertyDefSP spPropDef;
	checkResult(spCD_comp->RegisterOptionalPropertyDef (TEST_ZERO_PROP_ID,
		L"Zero String Property",
		spTD,
		&spPropDef));
	
	return S_OK;
}//addZeroStringPropertyToComponent()


static HRESULT createZeroString(IAAFDictionary* const pDict, IAAFFillerSP& spFill)
{
	...
	//From Class Def, get the Property Def
	IAAFPropertyDefSP spPD_comp;
	checkResult(spCD_comp->LookupPropertyDef(TEST_ZERO_PROP_ID, &spPD_comp));
	...
	//first, get the type def
	IAAFTypeDefSP spTypeDef;
	checkResult(pDict->LookupTypeDef(kAAFTypeID_String, &spTypeDef));
	...
	//Create zero size string as property value
	IAAFPropertyValueSP spPropVal;
	const aafUInt32 size = 0;
	aafCharacter* pZeroStr = new aafCharacter[size];
	checkResult( spSTR->CreateValueFromCString (
		(aafMemPtr_t)  pZeroStr,
		size,
		&spPropVal)	);
	delete [] pZeroStr;
							   ...
							   
{
	...
	/////  Verify Zero String Property

	// Assume that if it doesn't have the Zero String Property definition
	// the file was generated by a version of this unit test that doesn't
	// create the Zero String Property.

	//From Class Def, get the Zero String Property Def
	IAAFPropertyDefSP spZeroPD_comp;
	if (spCD_comp->LookupPropertyDef(TEST_ZERO_PROP_ID, &spZeroPD_comp) == S_OK)
	{
		aafBoolean_t  bIsZeroStrPresent = kAAFFalse;
		checkResult(spObj->IsPropertyPresent(spZeroPD_comp, &bIsZeroStrPresent));
		checkExpression(bIsZeroStrPresent == kAAFTrue, AAFRESULT_TEST_FAILED);

		//get the value
		IAAFPropertyValueSP spZeroPropVal;
		checkResult(spObj->GetPropertyValue(spZeroPD_comp, &spZeroPropVal));
		
		IAAFTypeDefSP spZeroType;
		checkResult(spZeroPropVal->GetType(&spZeroType));
...
#ifndef _DEBUG
		checkResult(addZeroStringPropertyToComponent (pDict));
#endif
...
#ifndef _DEBUG
		checkResult(createZeroString(pDict, spFill));
#endif
